...
async def test_build_skill_fingerprint_empty_repos():
...
async def test_build_skill_fingerprint_all_forked():
    repos = [
        {"name": "fork1", "language": "Python", "fork": True},
        {"name": "fork2", "language": "Rust", "fork": True},
    ]
    fp = await build_skill_fingerprint(repos)
    assert fp["total_repos"] == 0
    assert fp["languages"] == {}
...
async def test_issue_text_to_vector_with_none_labels():
    vec = await issue_text_to_vector("title", "body", None)
    assert len(vec) == 128
...
async def test_extract_required_skills(labels, expected_cat):
    skills = await extract_required_skills(
        ...
        "Use FastAPI and PostgreSQL for data processing. This is a standard feature.",
        labels,
    )
    if expected_cat:
        assert expected_cat in skills["categories"]
    assert isinstance(skills["complexity"], float)
